...
export default function EmbedNode({ data }) {
    const { deleteNode,onChangeNode,autoData,onEditChangeData } = useContext(AutomationContext);
    let val = autoData?.find(d => d.name === 'EmbedNode')?.value;
    ...
    return (
        <>
            ...
            <div className="wd-aside">
                <div className="card">
                    <h5 className="card-header bg-light">Embed Block
                    <button
                    ...
                    <div className="card-body">
                        <input type="text" name="EmbedNode" onBlur={(e)=>onChangeNode(e)} className="form-control form-lg" placeholder="Paste the link or code.."/>
                    </div>
                </div>
            </div>
