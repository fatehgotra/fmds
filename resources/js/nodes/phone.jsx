...
export default function PhoneNode({ data }) {
    const { deleteNode,onChangeNode,autoData,onEditChangeData } = useContext(AutomationContext);
    let val = autoData?.find(d => d.name === 'PhoneNode')?.value;
    return (
        <>
            ...
            <div className="wd-aside">
                <div className="card">
                    <h5 className="card-header bg-light">Phone Block
                    <button
                    ...
                    <div className="card-body">
                        <input type="tel" name="PhoneNode"  onBlur={(e)=>onChangeNode(e)} className="form-control" placeholder="Type a phone number.."/>
                    </div>
                </div>
            </div>
